...
def loans_test():
    ...
    applicant = loans.Applicant(">90", ["4"])
    assert repr(applicant) == "Applicant('>90', ['Native Hawaiian or Other Pacific Islander'])"
    applicant = loans.Applicant("<50", ["27"])
    assert repr(applicant) == "Applicant('<50', ['Other Asian'])"
    loans_points += 4
